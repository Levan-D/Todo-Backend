...
type TenantComparisonActInput struct {
	...
	Payments              []TenantComparisonActPaymentItem
}

type TenantComparisonActPaymentItem struct {
	TenantName             string
	...
	BankCommission         decimal.Decimal
	TenantSpentAfterPeriod decimal.Decimal
	MainCommission         decimal.Decimal
}

type TenantComparisonActData struct {
	TenantID    uuid.UUID
	GeneratedAt time.Time
	PeriodFrom  time.Time
	PeriodTo    time.Time
...
func (r report) GenerateReportTenantComparisonAct(id uuid.UUID) (string, TenantComparisonActData, error) {
	tenantInvoice, err := r.findComparisonActByID(id)
	if err != nil {
		return "", TenantComparisonActData{}, nil
	}

	payments, err := r.findPaymentByComparisonActID(id)
	if err != nil {
		return "", TenantComparisonActData{}, nil
	}

	paymentItems := []TenantComparisonActPaymentItem{}

	for _, payment := range payments {
		paymentItems = append(paymentItems, TenantComparisonActPaymentItem{
			...
			BankCommission:         payment.CommissionBank,
			TenantSpentAfterPeriod: payment.GetPendingAcquiredPoints(),
			MainCommission:         payment.CommissionAccount,
		})
	}
	...
	path, err := r.generateReportComparisonActTemplate(TenantComparisonActInput{
		...
		TotalCommission:       tenantInvoice.TotalCommissionBank,
		Payments:              paymentItems,
	})
	if err != nil {
		return "", TenantComparisonActData{}, err
	}

	return path, TenantComparisonActData{
...
func (r report) generateReportComparisonActTemplate(input TenantComparisonActInput) (path string, err error) {
	sheetName := fmt.Sprintf("COMPARISON ACT %s", input.GeneratedAt.Format("2006-01-02"))
	...
	xlsx.SetCellValue(sheetName, "A1", fmt.Sprintf("%s - შედარების აქტის დათვლის პერიოდი", input.TenantName))
	...
	xlsx.SetCellValue(sheetName, "A12", "საკომისიო")
	xlsx.SetCellValue(sheetName, "B12", input.TotalCommission)
	...
	for _, payment := range input.Payments {
		...
		xlsx.SetCellValue(sheetName, fmt.Sprintf("M%d", paymentRowIndex), payment.BankCommission)
		xlsx.SetCellValue(sheetName, fmt.Sprintf("N%d", paymentRowIndex), payment.MainCommission)
		paymentRowIndex++
	}
	...
	xlsxFilePath := fmt.Sprintf("%s/tenant-comparison-act-%s-%s-%s.xlsx", config.GetDirectoryPath("tmp"), tenantCode, input.PeriodFrom.Format("2006-02-01"), utils.GenerateRandomNumbers(5))
	//zippedFilePath := fmt.Sprintf("%s/comparison-act-%s-%s.zip", config.GetDirectoryPath("tmp"), strings.ToLower(string(input.TenantName)), input.PeriodFrom.Format("2006-02-01"))
...
func (r report) findComparisonActByID(id uuid.UUID) (domain.TenantComparisonAct, error) {
	var invoice domain.TenantComparisonAct
	err := r.DB.Preload("Tenant").Where("id = ?", id).First(&invoice).Error
	return invoice, err
}

func (r report) findPaymentByComparisonActID(invoiceId uuid.UUID) ([]domain.Payment, error) {
	var payments []domain.Payment
	err := r.DB.Preload("Tenant").Preload("Gateway").Where("tenant_comparison_act_id = ?", invoiceId).Find(&payments).Error
	return payments, err
}
