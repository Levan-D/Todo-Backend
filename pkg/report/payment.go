...
func (r report) GenerateReportPayment(from time.Time, to time.Time) (url string, err error) {
	...
	var totalStats PaymentStatistic
	...
	var paidStats PaymentStatistic
	...
	paymentList := make([]PaymentItem, 0, len(payments))
	...
	for _, payment := range payments {
		...
		if (payment.Status == domain.PaymentStatusPaid || payment.Status == domain.PaymentStatusReturned) && payment.Type != domain.PaymentTypeCardAdd {
			...
			paymentList = append(paymentList, PaymentItem{
	...
	path, err := r.generateReportPaymentTemplate(Input{
		PeriodFrom:                   from,
		PeriodTo:                     to,
		...
		Tenants:                      r.getReportPaymentTopTenantsByAmount(tenantPayments, tenantNames, 10),
...
func (r report) generateReportPaymentTemplate(input Input) (path string, err error) {
	...
	xlsx.SetCellValue(summarySheetName, "B6", BeautyDecimalForExcel(input.TotalSubTotal))

	xlsx.SetCellValue(summarySheetName, "A7", "გადახდილი ქულები")
	xlsx.SetCellValue(summarySheetName, "B7", BeautyDecimalForExcel(input.TotalUsedPoint))

	xlsx.SetCellValue(summarySheetName, "A8", "გადახდილი ლარი (ფასდაუკლებელი)")
	xlsx.SetCellValue(summarySheetName, "B8", BeautyDecimalForExcel(input.TotalAmount))

	xlsx.SetCellValue(summarySheetName, "A9", "გადახდილი ლარი (ფასდაკლებული)")
	xlsx.SetCellValue(summarySheetName, "B9", BeautyDecimalForExcel(input.TotalAmountDiscounted))

	xlsx.SetCellValue(summarySheetName, "A10", "დაგროვილი ქულები (ფასდაუკლებელი)")
	xlsx.SetCellValue(summarySheetName, "B10", BeautyDecimalForExcel(input.TotalAcquiredPoint))

	xlsx.SetCellValue(summarySheetName, "A11", "დაგროვილი ქულები (ფასდაკლებული)")
	xlsx.SetCellValue(summarySheetName, "B11", BeautyDecimalForExcel(input.TotalAcquiredPointDiscounted))

	xlsx.SetCellValue(summarySheetName, "A12", "")

	xlsx.SetCellValue(summarySheetName, "A13", "დაბრუნებული ქულები")
	xlsx.SetCellValue(summarySheetName, "B13", BeautyDecimalForExcel(input.ReturnedUsedPoint))

	xlsx.SetCellValue(summarySheetName, "A14", "დაბრუნებული ლარი (ფასდაუკლებელი)")
	xlsx.SetCellValue(summarySheetName, "B14", BeautyDecimalForExcel(input.ReturnedAmount))

	xlsx.SetCellValue(summarySheetName, "A15", "დაბრუნებული ლარი (ფასდაკლებული)")
	xlsx.SetCellValue(summarySheetName, "B15", BeautyDecimalForExcel(input.ReturnedAmountDiscounted))

	xlsx.SetCellValue(summarySheetName, "A16", "")

	xlsx.SetCellValue(summarySheetName, "A17", "ბანკის საკომისიო სულ:")
	xlsx.SetCellValue(summarySheetName, "B17", BeautyDecimalForExcel(input.TotalCommissionBank))

	xlsx.SetCellValue(summarySheetName, "A18", "გალერიას საკომისიო სულ:")
	xlsx.SetCellValue(summarySheetName, "B18", BeautyDecimalForExcel(input.TotalCommissionAccount))

	xlsx.SetCellValue(summarySheetName, "A19", "")

	xlsx.SetCellValue(summarySheetName, "A20", "ტენანტებთან გადასარიცხი თანხის ჯამი")
	xlsx.SetCellValue(summarySheetName, "B20", BeautyDecimalForExcel(input.TenantUnpaidTotal))

	xlsx.SetCellValue(summarySheetName, "A21", "ტენანტებთან გადარიცხული თანხის ჯამი")
	xlsx.SetCellValue(summarySheetName, "B21", BeautyDecimalForExcel(input.TenantPaidTotal))
	...
	xlsx.SetCellValue(summarySheetName, "B24", BeautyDecimalForExcel(input.ParkingTransactionTotal))
	...
	xlsx.SetCellValue(summarySheetName, "B28", BeautyDecimalForExcel(input.VisitorFemaleAmountSpent))
	...
	xlsx.SetCellValue(summarySheetName, "B30", BeautyDecimalForExcel(input.VisitorMaleAmountSpent))
	...
	for _, tenant := range input.Tenants {
		xlsx.SetCellValue(summarySheetName, fmt.Sprintf("A%d", tenantRowIndex), tenant.Name)
		xlsx.SetCellValue(summarySheetName, fmt.Sprintf("B%d", tenantRowIndex), BeautyDecimalForExcel(tenant.Value))
		tenantRowIndex++
	}
	...
	for _, payment := range input.Payments {
		...
		xlsx.SetCellValue(paymentSheetName, fmt.Sprintf("K%d", paymentRowIndex), BeautyDecimalForExcel(payment.Total))
		xlsx.SetCellValue(paymentSheetName, fmt.Sprintf("L%d", paymentRowIndex), BeautyDecimalForExcel(payment.UsedPoint))
		xlsx.SetCellValue(paymentSheetName, fmt.Sprintf("M%d", paymentRowIndex), BeautyDecimalForExcel(payment.Amount))
		xlsx.SetCellValue(paymentSheetName, fmt.Sprintf("N%d", paymentRowIndex), BeautyDecimalForExcel(payment.AmountDiscounted))
		xlsx.SetCellValue(paymentSheetName, fmt.Sprintf("O%d", paymentRowIndex), BeautyDecimalForExcel(payment.CommissionBank))
		xlsx.SetCellValue(paymentSheetName, fmt.Sprintf("P%d", paymentRowIndex), BeautyDecimalForExcel(payment.CommissionAccount))
		xlsx.SetCellValue(paymentSheetName, fmt.Sprintf("Q%d", paymentRowIndex), BeautyDecimalForExcel(payment.CashbackPercent))
		xlsx.SetCellValue(paymentSheetName, fmt.Sprintf("R%d", paymentRowIndex), BeautyDecimalForExcel(payment.CashbackDiscountedPercent))
		xlsx.SetCellValue(paymentSheetName, fmt.Sprintf("S%d", paymentRowIndex), BeautyDecimalForExcel(payment.Total.Sub(payment.AmountDiscounted)))
		xlsx.SetCellValue(paymentSheetName, fmt.Sprintf("T%d", paymentRowIndex), BeautyDecimalForExcel(payment.AcquiredPoint))
		xlsx.SetCellValue(paymentSheetName, fmt.Sprintf("U%d", paymentRowIndex), BeautyDecimalForExcel(payment.AcquiredPointDiscounted))
		xlsx.SetCellValue(paymentSheetName, fmt.Sprintf("V%d", paymentRowIndex), payment.AcquiredPointStatus)
		xlsx.SetCellValue(paymentSheetName, fmt.Sprintf("W%d", paymentRowIndex), BeautyDecimalForExcel(payment.TenantSpentAmount))
...
func BeautyDecimalForExcel(val decimal.Decimal) string {
	return strings.ReplaceAll(val.Round(2).String(), ".", ",")
}

func (r report) getReportPaymentTotalSpent(genderSpent map[uuid.UUID]decimal.Decimal) decimal.Decimal {
...
func (r report) getReportPaymentTopTenantsByAmount(payments map[uuid.UUID]decimal.Decimal, names map[uuid.UUID]string, n int) []TenantItem {
